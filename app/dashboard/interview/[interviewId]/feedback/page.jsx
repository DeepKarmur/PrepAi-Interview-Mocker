...
function Feedback({params}) {
  ...
  return (
    <div className='p-10'>
       ...
       <h2 className='text-blue-400 text-lg my-3'>Your Overall rating: 
        <strong>6/10</strong></h2>
       
       <h2 className='text-sm text-gray-500'>Find below interview question with correct answer, Your answer and feedback for improvement</h2>
       {/* {feedbackList && feedbackList.map((item, index) => (
                <Collapsible key={index}>
                    <CollapsibleTrigger>{item.question || "Question"}</CollapsibleTrigger>
                    <CollapsibleContent>
                        Your Answer: {item.yourAnswer || "N/A"}
                        <br />
...
{feedbackList&&feedbackList.map((item,index)=>(
        ...
        <Button onClick={() => router.replace('/dashboard')} className='bg-blue-500 hover:bg-blue-300 text-white'>Go Home</Button>
    </div>
  )
}
